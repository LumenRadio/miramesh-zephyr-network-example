...
#define TRAILER_PAGE_ADDRESS(slot_address) (IMAGE_TRAILER_PAGE_OFFSET + (slot_address) - (SWAP_SIZE - FLASH_PAGE_SIZE))
...
static bool is_overlapping_mira_fota_header(uint32_t address, uint32_t length) {
    if ((address + length <= MIRA_HEADER_LOCATION) || (address >= MIRA_HEADER_LOCATION + MIRA_FOTA_HEADER_SIZE)) {
        return false;
    } else {
        return true;
    }
                     ...
                     void* storage)
{
    if (slot_id == 0) {
        ...
        if (address == 0 && length == MIRA_FOTA_HEADER_SIZE) {
            ...
            LOG_DBG("Read from slot 0, Mira header, addr: %d, length: %d", MIRA_HEADER_LOCATION, length);
            flash_read(header_dev, HEADER_PAGE_ADDRESS(MIRA_HEADER_LOCATION), img_fragment, MIRA_FOTA_HEADER_SIZE);
            done_callback(storage);
            return 0;
        }
        ...
        if (address_in_header_page(address)) {
            ...
            if (overlap == 0) {
                ...
                LOG_DBG("Read from slot 0, header overlapping, addr: %d, length: %d", address, length);
            ...
            if(overlap == 0) {
                ...
                LOG_DBG("Read from slot 0, trailer, overlapping addr: %d, length: %d", address, length);
                ...
                flash_read(trailer_dev, TRAILER_PAGE_ADDRESS(address + length_in_swap), img_fragment, overlap);
            }
        } else {
                      ...
                      void* storage)
{
    if (slot_id == 0) {
        ...
        if (address == 0 && length == MIRA_FOTA_HEADER_SIZE) {
            const struct device *header_dev = IMAGE_HEADER_PAGE_DEVICE;
            LOG_DBG("Write to slot 0, Mira header, addr: %d, length: %d", MIRA_HEADER_LOCATION, length);
            flash_write(header_dev, HEADER_PAGE_ADDRESS(MIRA_HEADER_LOCATION), img_fragment, MIRA_FOTA_HEADER_SIZE);
            done_callback(storage);
            return 0;
        }
        ...
        if (address_in_header_page(address)) {
            ...
            if (overlap == 0) {
                if (is_overlapping_mira_fota_header(address, length)) {
                    ...
                    if (after != 0) {
                        flash_write(header_dev, HEADER_PAGE_ADDRESS(address + before + MIRA_FOTA_HEADER_SIZE), &img_fragment[before + MIRA_FOTA_HEADER_SIZE], after);
                    }
                ...
                flash_write(swap_dev, SWAP_ADDRESS(address), img_fragment, length);
                uint32_t length_in_header = length - overlap;
                flash_write(header_dev, HEADER_PAGE_ADDRESS(address), img_fragment, length_in_header);
            }
        } else if (address_in_trailer_page(address, length)) {
            const struct device *trailer_dev = IMAGE_TRAILER_PAGE_DEVICE;
            uint32_t overlap = flash_check_overlapping(address, length);
            if (overlap == 0) {
                ...
                LOG_DBG("Write to slot 0, trailer overlapping, offset: %d, length: %d", address, length);
                flash_write(swap_dev, SWAP_ADDRESS(address), img_fragment, length);
                uint32_t length_in_swap = length - overlap;
                img_fragment += length_in_swap;
                LOG_DBG("length_in_swap: %u, overlap: %u", length_in_swap, overlap);
                flash_write(trailer_dev, TRAILER_PAGE_ADDRESS(address + length_in_swap), img_fragment, overlap);
            }
        } else {
...
K_THREAD_DEFINE(fota_swap_erase_worker_thread_id, 1024, fota_swap_erase_worker, NULL, NULL, NULL, 8, 0, 0);
